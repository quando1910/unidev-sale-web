	...
	$.AngoraTheme = {
		...
		navigation:function() {
			...
			$('a[href^=\\#]').on("click", function(e) {
				if ($(this).attr('href')!=='#' && !$(e.target).parent().parent().is('.navbar-nav') && !$(this).attr('data-toggle')) {
					$(document).scrollTo($(this).attr('href'), AngoraOptions.scrollSpeed, {offset:{top:-85, left:0}});
					e.preventDefault();
				}
			});
		...
		intro:function() {
			...
			//Fullscreen Video
				...
				if (Modernizr.touch) {
					...
					$('<div />').addClass('image').css({
						opacity:0,
						backgroundImage:"url('"+$intro.attr('data-on-error')+"')",
						backgroundRepeat:'no-repeat'
					}).appendTo('.slider');
					...
					$(document).ready(function() {
						...
						var quality = $intro.attr('data-quality');
		...
		portfolio:function() {
			...
			$('a[href^="#view-"]').on("click", function() {
				...
				if ($item.length>0) {
					$(document).scrollTo('#portfolio', AngoraOptions.scrollSpeed, {offset:{top:-85, left:0}, onAfter:function() {
						$item.trigger('click');
					}});
				}
			});
